...
import { retryLocalMockupJob } from "@/lib/jobs/local-mockup-runner";

export async function POST(
  request: Request,
  { params }: { params: Promise<{ jobId: string }> }
) {
  const auth = await requireAppUser(request, "retrying mockup jobs");
  ...
  const result = await retryLocalMockupJob(jobId, auth.firestoreUser.id);
